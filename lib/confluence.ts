...
export async function fetchUseCaseDetails(pageId: string) {
	...
	return tables.map((table, index) => {
		...
		const authorName = parsed.structuredText.split('\n').find((line) => line.includes('Your name:'))?.replace(/Your name:/, '').trim();
		const customerName = parsed.structuredText.split('\n').find((line) => line.includes('Customer:'))?.replace(/Customer:/, '').trim();
		const date = parsed.structuredText.split('\n').find((line) => line.includes('Date:'))?.replace(/Date:/, '').trim();
